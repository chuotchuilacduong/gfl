...
class NodeClsTask(BaseTask):
    ...
    def __init__(self, args, client_id, data, data_dir, device):
        ...
        Args:
            ...
            device (torch.device): Device to run the computations on.
        """
        self.syn_data_global = Data().to(device)
        super(NodeClsTask, self).__init__(args, client_id, data, data_dir, device)

    def train(self, splitted_data=None, G_syn_global=None):
        """
        ...
        for _ in range(self.args.num_epochs):
            ...
            embedding_local, logits_local = self.model.forward(splitted_data["data"])           
            loss_local = self.loss_fn(embedding_local, logits_local, 
                                      splitted_data["data"].y, 
                                      splitted_data["train_mask"])
            
            if self.args.dp_mech != "no_dp":
                clip_gradients(self.model, loss_local, loss_local.shape[0], self.args.dp_mech, self.args.grad_clip)
            else:
                loss_local.backward()
            ...
            self.optim.step() 
            if G_syn_global is not None:
                self.optim.zero_grad()
                
                x_syn = G_syn_global["syn_x"]
                adj_syn_norm = G_syn_global["adj_syn_norm"]
                y_syn = G_syn_global["syn_y"]
                edge_index_syn, edge_weight_syn = dense_to_sparse(adj_syn_norm)
                
                self.syn_data_global.x = x_syn
                self.syn_data_global.y = y_syn
                self.syn_data_global.edge_index = edge_index_syn
                

                embedding_syn, logits_syn = self.model.forward(self.syn_data_global)
                loss_global = self.loss_fn(embedding_syn, logits_syn, y_syn, 
                                            torch.ones_like(y_syn).bool())
                
                loss_global.backward() 
                self.optim.step()   

            if self.args.dp_mech != "no_dp":
                add_noise(self.args, self.model, loss_local.shape[0])
