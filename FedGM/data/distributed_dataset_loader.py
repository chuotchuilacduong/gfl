...
class FGLDataset(Dataset):
    ...
    def check_args(self, args):
        ...
        if args.scenario == "graph_fl":
            from config import supported_graph_fl_datasets, supported_graph_fl_simulations, supported_graph_fl_tasks
            for dataset in args.dataset:
                assert dataset in supported_graph_fl_datasets, f"Invalid graph-fl dataset '{dataset}'."
            assert args.simulation_mode in supported_graph_fl_simulations, f"Invalid graph-fl simulation mode '{args.simulation_mode}'."
            assert args.task in supported_graph_fl_tasks, f"Invalid graph-fl task '{args.task}'."
            
            
        elif args.scenario == "subgraph_fl":
            from config import supported_subgraph_fl_datasets, supported_subgraph_fl_simulations, supported_subgraph_fl_tasks
            for dataset in args.dataset:
                assert dataset in supported_subgraph_fl_datasets, f"Invalid subgraph-fl dataset '{dataset}'."
            ...
            assert args.task in supported_subgraph_fl_tasks, f"Invalid graph_fl task '{args.task}'."
        elif args.scenario in ["subgraph_fl_hetero", "subgraph_fl_kg"] :
            from config import supported_subgraph_fl_hetero_datasets, supported_subgraph_fl_hetero_simulations, supported_subgraph_fl_hetero_tasks
            for dataset in args.dataset:
                assert dataset in supported_subgraph_fl_hetero_datasets, f"Invalid subgraph-fl heterogeneous dataset '{dataset}'."
            assert args.simulation_mode in supported_subgraph_fl_hetero_simulations, f"Invalid subgraph-fl heterogeneous simulation mode '{args.simulation_mode}'."
            assert args.task in supported_subgraph_fl_hetero_tasks, f"Invalid fedgraph task '{args.task}'."
