...
def load_client(args, client_id, data, data_dir, message_pool, device):
    ...
    elif args.fl_algorithm == "fedgm":
        from flcore.fedgm.client import FedGMClient
        return FedGMClient(args, client_id, data, data_dir, message_pool, device)
    elif args.fl_algorithm == "fedgm_hybrid":
        from flcore.fedgm_hybrid.client import FedGMHybridClient
        return FedGMHybridClient(args, client_id, data, data_dir, message_pool, device)
...
def load_server(args, global_data, data_dir, message_pool, device):
    if args.fl_algorithm == "isolate":
        ...
        return FedGMServer(args, global_data, data_dir, message_pool, device)
    elif args.fl_algorithm == "fedgm_hybrid":
        from flcore.fedgm_hybrid.server import FedGMHybridServer
        return FedGMHybridServer(args, global_data, data_dir, message_pool, device)
    elif args.fl_algorithm == "fedrgd":
