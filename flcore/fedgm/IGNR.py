...
def mx_inv(mx):
    ...
    if D_min < eps:
        ...
        D_inv = 1.0 / D
    return torch.mm(torch.mm(U, torch.diag(D_inv)), V.t())
...
def mx_inv_sqrt(mx):
    ...
    D_min = torch.min(D)
    if D_min < eps:
        D_inv_sqrt = 1.0 / (torch.sqrt(D + eps))
    else:
        D_inv_sqrt = 1.0 / torch.sqrt(D)
    return torch.mm(torch.mm(U, torch.diag(D_inv_sqrt)), V.t()), mx_inv(mx)

def mx_tr(mx):
    return torch.trace(mx)
