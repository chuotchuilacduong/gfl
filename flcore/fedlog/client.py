...
class FedLogClient(BaseClient):
    def __init__(self, args, client_id, data, data_dir, message_pool, device):
        ...
        # 1. Load dữ liệu đồ thị và thông tin cơ bản
        self.local_graph = self.task.data.to(self.device)
        self.num_features = self.local_graph.x.shape[1]
        
        # Gán các mask từ task vào local_graph
        for mask_name in ['train_mask', 'val_mask', 'test_mask']:
            if hasattr(self.task, mask_name):
                setattr(self.local_graph, mask_name, getattr(self.task, mask_name))

        # 2. Xác định số lượng lớp (Sử dụng num_clusters từ config.py)
        # Trong FedLogClient.__init__
        # Lấy nhãn lớn nhất thực tế trong dữ liệu cục bộ để kiểm tra
        actual_max_label = int(self.local_graph.y.max().item()) + 1

        # 1. Xác định số lượng lớp TOÀN CỤC (Dùng num_clusters từ config.py)
        # Bạn PHẢI đảm bảo tham số --num_clusters truyền vào khớp với dataset
        if hasattr(self.args, 'num_clusters'):
            self.num_classes = self.args.num_clusters
        else:
            # Nếu không có num_clusters, fallback này chỉ an toàn nếu các client có đủ nhãn
            self.num_classes = int(self.local_graph.y.max().item() + 1)
            
        self.global_num_classes = self.num_classes

        # 2. Khởi tạo mô hình với num_classes cố định này
        self.classifier = Linear(self.args.hid_dim, self.num_classes).to(self.device)        
        self.global_num_classes = self.num_classes
                
        # 3. Khởi tạo Mô hình (Sử dụng hid_dim, num_layers, dropout từ config.py)
        self.embedder = GraphSAGE(
            in_channels=self.num_features, 
            hidden_channels=self.args.hid_dim * 2, 
            out_channels=self.args.hid_dim, 
            dropout=self.args.dropout, 
            num_layers=self.args.num_layers
        ).to(self.device)
        
        self.clsf_head = clsf(self.args.hid_dim, self.args.hid_dim, n_layers=2).to(self.device)
        self.clsf_tail = clsf(self.args.hid_dim, self.args.hid_dim, n_layers=2).to(self.device)
        self.classifier = Linear(self.args.hid_dim, self.num_classes).to(self.device)
        
        # 4. Neighbor Generators
        self.neigh_gen = neighGen(self.num_features, self.args).to(self.device)
        self.neigh_cls_gen = torch.nn.ModuleList([
            neighGen(self.num_features, self.args).to(self.device) for _ in range(self.num_classes)
        ])
        
        # 5. Thiết lập dữ liệu tổng hợp (Sử dụng num_proto từ config.py)
        self.n_way = self.num_classes
        self.k_shot = self.args.num_proto
        total_syn_nodes = int(self.n_way * self.k_shot)
        
        syn_y = []
        for cls in range(self.num_classes): 
            syn_y += [cls] * self.k_shot
        self.syn_y = torch.LongTensor(syn_y).to(self.device)
        
        self.syn_adj = SparseTensor.eye(total_syn_nodes, total_syn_nodes).t().to(self.device)

        # Khởi tạo Synthetic Features dưới dạng Parameter (Sửa lỗi logic: Khởi tạo 1 lần duy nhất)
        self.syn_feat_head = torch.nn.Parameter(torch.zeros(total_syn_nodes, self.num_features).to(self.device))
        self.syn_feat_tail = torch.nn.Parameter(torch.zeros(total_syn_nodes, self.num_features).to(self.device))
        torch.nn.init.xavier_uniform_(self.syn_feat_head.data)
        torch.nn.init.xavier_uniform_(self.syn_feat_tail.data)

        # 6. Các scaler và buffer toàn cục
        ...
        # 7. Khởi tạo Optimizers (Sử dụng lr và weight_decay từ config.py)
        ...
        # Optimizer cho Synthetic Features (Phải trỏ vào Parameter đã khởi tạo ở trên)
    ...
    def train_model(self, cur_round):
        ...
        # TỐI ƯU: Tính alpha gating một lần duy nhất ngoài vòng lặp
        with torch.no_grad():
            node_deg = degree(self.local_graph.edge_index[0], num_nodes=self.local_graph.x.shape[0])
            alpha_all = 1 / (1 + torch.exp(-(node_deg - (self.args.head_deg_thres + 1))))
            alpha_train = alpha_all[train_mask].unsqueeze(1).to(self.device)
        ...
        for epoch in range(self.args.local_epochs):
            ...
            # 1. Synthetic Embeddings & Prototypes
            syn_h_head = self.embedder(self.syn_feat_head, self.syn_adj)
            syn_proto_head = syn_h_head.view([self.n_way, self.k_shot, -1]).mean(1)
            
            syn_h_tail = self.embedder(self.syn_feat_tail, self.syn_adj)
            syn_proto_tail = syn_h_tail.view([self.n_way, self.k_shot, -1]).mean(1)

            # 2. Global Prototype (Nếu từ round 1 trở đi)
            # 2. Global Prototype (Tính toán trọng số từ tri thức toàn cục)
            global_proto = None 
            if cur_round > 0 and self.merged_global_graph is not None:
                with torch.no_grad():
                    merged_g = self.merged_global_graph
                    global_h = self.embedder(merged_g.x, merged_g.edge_index)
                    global_proto = torch.zeros(self.num_classes, h.size(1), device=self.device)
                    
                    # Số lượng node mà mỗi client đóng góp vào đồ thị gộp
                    nodes_per_client = self.num_classes * self.args.num_proto
                    
                    for cls_label in range(self.num_classes):
                        # Lấy tất cả node có nhãn cls_label trong đồ thị gộp
                        class_indices = (merged_g.y == cls_label).nonzero(as_tuple=True)[0]
                        if len(class_indices) == 0: continue
                        
                        # BƯỚC SỬA LỖI: Ánh xạ node_index -> client_id -> rate_index
                        # a. Xác định client sở hữu từng node
                        client_ids = class_indices // nodes_per_client
                        
                        # b. Xác định vị trí của rate tương ứng trong vector cls_rate
                        # Mỗi client có num_classes phần tử rate xếp liên tiếp
                        rate_indices = client_ids * self.num_classes + cls_label
                        
                        # c. Lấy giá trị rate (đảm bảo rate_indices nằm trong bounds của cls_rate)
                        client_cls_rate = merged_g.cls_rate[rate_indices]

                        # d. Chuẩn hóa trọng số để tổng bằng 1 (Weighted Average)
                        sum_rate = client_cls_rate.sum()
                        if sum_rate == 0:
                            client_cls_rate = torch.ones_like(client_cls_rate) / len(client_cls_rate)
                        else:
                            client_cls_rate = client_cls_rate / sum_rate
                        
                        # e. Tính toán Prototype toàn cục bằng tổng có trọng số
                        global_proto[cls_label] = (global_h[class_indices] * client_cls_rate.unsqueeze(1)).sum(0)
            # 3. Cấu trúc Metric Learning Graph
            current_batch_size = len(h[train_mask])
            start_idx = self.n_way * self.k_shot
            edge1 = torch.arange(start_idx, start_idx + current_batch_size).repeat_interleave(start_idx)
            edge2 = torch.arange(start_idx).repeat(current_batch_size)
            ...
            neighbor = neighbor / adj_matrix.sum(0).clamp(min=1).unsqueeze(1)
            
            # 5. Task Adaptation & Prediction
            _, embeds_epi_head = self.clsf_head(h[train_mask], neighbor[train_mask], syn_h_head, metric_edge_index)
            _, embeds_epi_tail = self.clsf_tail(h[train_mask], neighbor[train_mask], syn_h_tail, metric_edge_index)

            out_head = F.log_softmax(-euclidean_dist(embeds_epi_head, syn_proto_head), dim=1)
            out_tail = F.log_softmax(-euclidean_dist(embeds_epi_tail, syn_proto_tail), dim=1)
            
            output_metric = alpha_train * out_head + (1 - alpha_train) * out_tail
            loss_metric = loss_fn(output_metric, y_train)
            
            syn_norm_loss = 0.5 * (torch.norm(self.syn_feat_head) + torch.norm(self.syn_feat_tail))

            # 6. Cross Domain Loss (CD)
            loss_cd_metric = 0
            if cur_round > 0 and self.global_synthetic_data is not None:
                g_syn_data = self.global_synthetic_data
                syn_xs = g_syn_data.x.to(self.device)
                ...
                # Generate Neighbors for CD Data
                syn_graph_list = []
                with torch.no_grad():
                    for cls_idx in range(self.num_classes):
                        ...
                        gen_feats = self.neigh_cls_gen[cls_idx](cls_inputs)
                        for k in range(gen_feats.shape[0]):
                            neigh_x = torch.cat([cls_inputs[k:k+1], gen_feats[k:k+1]], 0)
                            syn_graph_list.append(Data(x=neigh_x, edge_index=torch.LongTensor([[0,1],[1,0]]).to(self.device)))
                ...
                if syn_graph_list:
                    ...
                    # CD Metric Indexing
                    n_syn = len(global_h)
                    edge1_cd = torch.arange(start_idx, start_idx + n_syn).repeat_interleave(start_idx)
                    edge2_cd = torch.arange(start_idx).repeat(n_syn)
            ...
            # 7. Backward & Step
            total_loss = self.args.hyper_metric * loss_metric + self.args.hyper_syn_norm * syn_norm_loss
            if cur_round > 0: total_loss += self.args.hyper_cd_metric * loss_cd_metric
            ...
            epoch_acc += accuracy(output_metric.detach(), y_train)

        # 8. Cập nhật kết quả và chuẩn bị CondGraph
