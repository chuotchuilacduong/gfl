...
def get_dataset(dataset, normalize_features=False, transform=None):
...
class Pyg2Dpr(Dataset):#input dataset and get the divided one. if we input partitioned dataset, then we can get what we want
    def __init__(self, pyg_data, **kwargs):
        pyg_data = pyg_data
        n = pyg_data['data'].num_nodes
        self.adj = sp.csr_matrix((np.ones(pyg_data['data'].edge_index.shape[1]),#化为普通的稀疏矩阵
            (pyg_data['data'].edge_index[0].cpu(), pyg_data['data'].edge_index[1].cpu())), shape=(n, n))
        self.features = pyg_data['data'].x.cpu().numpy()
        self.labels = pyg_data['data'].y.cpu().numpy()
        ...
        self.idx_train = mask_to_index(pyg_data['train_mask'].cpu(), n)
        self.idx_val = mask_to_index(pyg_data['val_mask'].cpu(), n)
        self.idx_test = mask_to_index(pyg_data['test_mask'].cpu(), n)
        self.name = 'Pyg2Dpr'
